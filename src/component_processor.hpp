...
namespace ComponentProcessor {
    std::vector<basic_graph_t> generate_connected_components(const basic_graph_t &input_graph) {
        /* init mapping structures */
        auto component_map = std::vector<int>(boost::num_vertices(input_graph));
        auto vertex_map = std::vector<basic_graph_t::vertex_descriptor>(boost::num_vertices(input_graph));
        ...
        std::vector<basic_graph_t> components = std::vector<basic_graph_t>(num_connected_components);
